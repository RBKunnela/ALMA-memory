...
class TestALMAMCPServer:
    ...
    async def test_tools_list_request(self, mcp_server: ALMAMCPServer):
        ...
        # Should have 16 tools (8 core + 8 workflow tools added in v0.6.0)
        assert len(tools) == 16
        ...
        assert "alma_consolidate" in tool_names
        # Workflow tools (v0.6.0)
        assert "alma_checkpoint" in tool_names
        assert "alma_resume" in tool_names
        assert "alma_merge_states" in tool_names
        assert "alma_workflow_learn" in tool_names
        assert "alma_link_artifact" in tool_names
        assert "alma_get_artifacts" in tool_names
        assert "alma_cleanup_checkpoints" in tool_names
        assert "alma_retrieve_scoped" in tool_names
