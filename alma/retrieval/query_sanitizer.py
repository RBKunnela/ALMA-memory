...
Adapted from MemPalace query_sanitizer.py (MIT License).
...
near-total retrieval failure (89.8% -> 1.0% R@10). See MemPalace Issue #333.
...
def sanitize_query(raw_query: str) -> dict:
    ...
    AI agents sometimes prepend system prompts to search queries, which
    overwhelms the embedding and causes retrieval failure. This function
    applies a 4-step extraction pipeline to recover the actual query.
    ...
    def _strip_wrapping_quotes(candidate: str) -> str:
        """Remove wrapping quotes from a candidate string."""
    ...
    def _trim_candidate(candidate: str) -> str:
        """Trim a candidate to fit within MAX_QUERY_LENGTH."""
    ...
    # Look for question marks in segments (prefer later ones = more likely
    # the actual query)
    question_sentences = []
