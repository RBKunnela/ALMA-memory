"""
ALMA Ingestion — Memory Extractor.

Adapted from MemPalace (MIT License).

Extracts 5 types of memories from text using regex heuristics:
    1. DECISIONS    -- choices made ("we went with X because Y")
    2. PREFERENCES  -- user rules ("always use X", "never do Y")
...
_CODE_LINE_PATTERNS = [
    re.compile(r"^\s*[\$#]\s"),
    re.compile(
        r"^\s*(cd|source|echo|export|pip|npm|git|python|bash|curl|wget|"
        r"mkdir|rm|cp|mv|ls|cat|grep|find|chmod|sudo|brew|docker)\s"
    ),
    re.compile(r"^\s*```"),
...
def _score_markers(text: str, markers: List[str]) -> Tuple[float, List[str]]:
    ...
    Args:
        text: Text to score.
        markers: List of regex patterns.
...
def extract_memories(text: str, min_confidence: float = 0.3) -> List[Dict]:
    ...
    Uses regex heuristics to classify text segments into 5 memory
...
def _split_into_segments(text: str) -> List[str]:
    ...
    Tries speaker-turn splitting first (> markers, "Human:",
    "Assistant:", etc.), then falls back to paragraph splitting.
    """
    lines = text.split("\n")
